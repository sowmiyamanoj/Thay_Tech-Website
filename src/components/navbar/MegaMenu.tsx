...
import { FaCode, FaCloud, FaExternalLinkAlt, FaBook, FaCogs } from "react-icons/fa";
...
const MegaMenu = React.forwardRef<HTMLDivElement, MegaMenuProps>(({ isOpen, onMouseEnter, onMouseLeave }, ref) => {
  return (
    <div
      ref={ref}
      className={`absolute top-full left-1/2 transform -translate-x-1/2 w-3/4 border-gray-200 shadow-lg border-y ${isOpen ? "block" : "hidden"} z-50 rounded-lg`}
      style={{
        marginTop: "5px",
        paddingTop: "0px",
      ...
      <div className="flex flex-wrap max-w-screen-xl px-4 py-5 mx-auto text-sm text-gray-500">
        ...
        <div className="flex flex-wrap w-full gap-y-8">
          {menuItems.slice(0, 3).map((item) => (
            <div key={item.href} className="w-full md:w-1/3 space-y-4">
              <a href={item.href} className="flex items-center space-x-2 group navbar-hover-text">
                <h3 className="text-lg font-medium flex items-center group-hover:text-[#00D1F9]">
                  {item.icon && <span className="mr-2 text-gray-500 group-hover:text-[#00D1F9]">{item.icon}</span>}
                  {item.text}
                </h3>
              </a>
              {item.submenus && (
                <ul className="space-y-2 mt-2">
                  {item.submenus.map((submenu) => (
                    <li key={submenu.href} className="flex items-center group">
                      <svg
                      ...
                      <a href={submenu.href} className="text-gray-900 group-hover:text-[#00D1F9]">
                        {submenu.text}
                      </a>
                    </li>
                  ))}
        ...
        <div className="flex flex-wrap w-full gap-y-8 mt-8">
          ...
          <div className="w-full md:w-1/3 lg:w-1/3">
            <div className="relative w-full h-36 rounded-lg overflow-hidden border-2 border-[var(--heading-text)] bg-cover bg-center" style={{ backgroundImage: "url(/development/img9.jpg)" }}>
              {/* Dark overlay */}
